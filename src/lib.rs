...
#[derive(Deserialize)]
pub struct TodoGenerateRequest {
    pub prompt: String,
}
...
pub async fn generate_anthropic_response(
    ...
    opts.set_body(Some(&JsValue::from_str(&body_str)));

    let request = Request::new_with_str_and_init("https://api.anthropic.com/v1/messages", &opts)?;
    ...
    let data: AnthropicResponse = serde_wasm_bindgen::from_value(json)
    ...
    if let Some(content) = data.content.and_then(|mut c| c.pop()) {
        ...
        Err(JsValue::from_str("No content returned from API"))
    }
}

#[wasm_bindgen]
pub async fn generate_todo_response(prompt: &str, api_key: &str) -> Result<String, JsValue> {
